...
// payment-service/services/paymentServices.js
// services/paymentServices.js (payment-service)
const axios = require("axios");
...
class PaymentServices {
  async initialize({ orderId, userId, amount, email, gateway, redirectUrl }) {
    if (!orderId || !userId) throw new Error("orderId and userId required");
    ...
    let paymentUrl;
    let reference = `order_${orderId}_${Date.now()}`;
    ...
    if (gateway === "paystack") {
      const payload = {
        ...
        reference,
        callback_url: `${redirectUrl || process.env.BASE_URL}/api/payments/webhook/paystack`,
      };
      ...
      const payload = {
        ...
        currency: "NGN",
        redirect_url: `${redirectUrl || process.env.BASE_URL}/api/payments/webhook/flutterwave`,
        customer: { email: email || "no-reply@example.com" },
        meta: { orderId },
      };
  ...
  async handleWebhookPaystack(rawBody, signatureHeader) {
    const secret = process.env.PAYSTACK_WEBHOOK_SECRET || process.env.PAYSTACK_SECRET_KEY;
    const hash = crypto.createHmac("sha512", secret).update(rawBody).digest("hex");
    if (signatureHeader !== hash) throw new Error("Invalid Paystack signature");
    const payload = JSON.parse(rawBody);
    const data = payload.data;
    const reference = data.reference;
    ...
    // publish event for order-service to act upon
    await this.publishPaymentEvent(success, reference, payload, payment.orderId);
    return { handled: true };
  }

  // similar for Flutterwave...
}
